...
btnClearItems.addEventListener("click", () => {
    ...
    if (localStorage.getItem('records')) {
        alert('Records stored in localStorage will be deleted.');
...
btnSaveStorage.addEventListener("click", () => {
    // Convert the array of records to a string using JSON.stringify()
    const recordsString = JSON.stringify(arrRecords);
    
    // Store the string representation of the array in localStorage under a specific key
    localStorage.setItem('records', recordsString);
    
    // Provide some feedback to the user
    alert("Records saved to localStorage!");
});
// check if there are records as page loads
window.addEventListener('load', () => {
    const savedRecordsString = localStorage.getItem('records');
    
    // if true, parse string to array
    if (savedRecordsString) {
        arrRecords = JSON.parse(savedRecordsString);
        
        
        iterateRecords();
        
        
        alert("Records loaded from localStorage!");
    }
});
...
sortName.addEventListener("change", () =>{
  ...
  sortLetter.addEventListener("change", () =>{
    if (sortLetter.value == "A"){
      arrRecords.sort((a, b) => a.fname.localeCompare(b.fname));
      console.log("A-Z");
    }
    else if(sortLetter.value == "Z"){
...
function iterateRecords() {
    ...
    if(!(arrRecords.length == 0)) {
        ...
        document.getElementById("sortLetter").style.display = "inline";
        document.getElementById("btnSaveStorage").style.display = "inline";
